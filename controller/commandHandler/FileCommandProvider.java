...
public class FileCommandProvider implements controller.commandHandler.CommandProvider {
    ...
    public void registerCommands (controller.commandHandler.CommandRegistry registry){
        ...
        registry.register("login -u (?<username>\\S+) -p (?<password>\\S+)(?<stayLoggedIn> -stay-logged-in)?" ,
                new controller.commands.LoginMenuCommands.Login(menuManager));
        registry.register("forget password -u (?<username>\\S+) -e (?<email>\\S+)" ,
                new controller.commands.LoginMenuCommands.ForgetPassword(menuManager));
        registry.register("answer -a (?<answer>.+)" ,
                new controller.commands.LoginMenuCommands.Answer(menuManager));
        ...
        registry.register("show (?<allORavailable>all|available) plants" ,
                new controller.commands.PlantsList.ShowPlantsList(menuManager));
        registry.register("add plant -t (?<type>.+)" ,
                new controller.commands.PlantsList.AddPlant(menuManager));
        registry.register("remove plant -t (?<type>.+)" ,
                new controller.commands.PlantsList.RemovePlant(menuManager));
        registry.register("boost plant -t (?<type>.+)" ,
                new controller.commands.PlantsList.BoostPlant(menuManager));
        registry.register("start game" , new StartGame(menuManager));
        registry.register("advance time -t (?<count>\\d+) ticks" ,
                new controller.commands.MechanismsCommands.AdvancedTime(menuManager));
        registry.register("collect sun -l \\((?<x>\\d+), (?<y>\\d+)\\)" ,
                new controller.commands.MechanismsCommands.CollectSun(menuManager));
        registry.register("show sun amount" ,
                new controller.commands.MechanismsCommands.ShowSunAmount(menuManager));
